...
import { 
  ...
  Edit3, Save, AlertCircle, History 
...
const initialize = async () => {
  ...
  // 1. Получаем имя сотрудника из БД
  if (props.userId) {
...
const cancelBooking = async (shift) => {
  ...
  const confirmText = props.userRole === 'admin' 
...
const addToDraft = () => {
  // 1. Валидация
  if (!form.value.date || !form.value.start_time || !form.value.end_time) return;

  // 2. Проверка дублей (среди существующих И среди новых)
  const isDuplicate = 
    [...shifts.value, ...unsavedNewShifts.value]
...
const saveAllChanges = async () => {
  ...
  try {
    ...
    if (unsavedNewShifts.value.length > 0) {
      const recordsToInsert = unsavedNewShifts.value.map(({ id, ...rest }) => rest);
...
<template>
  <div class="pb-32"> <div class="px-4 py-3 flex justify-between items-end sticky top-0 bg-slate-50 z-30">
    ...
    <div v-if="loading" class="text-center py-10 opacity-50">
      ...
      <span class="text-[10px] font-bold uppercase">Загрузка расписания...</span>
    </div>
    ...
    <div v-else class="space-y-6 px-3">
      <div v-for="(dayShifts, date) in groupedShifts" :key="date">
        ...
        <div class="flex items-center gap-2 mb-2 ml-1">
          ...
          <button v-if="userRole === 'admin'" @click="openModal(null, date)" class="text-slate-300 hover:text-blue-500 p-1">
            <Plus class="w-3.5 h-3.5" />
          </button>
        </div>

        <div class="space-y-2">
          <div v-for="shift in dayShifts" :key="shift.id" 
               ...
               :class="shift.id < 0 ? 'border-blue-300 bg-blue-50/30' : 'border-slate-100'">
            
            <div class="flex items-center gap-2 z-10">
              <div class="px-2 py-1 rounded text-[11px] font-black text-slate-600 border border-slate-100 bg-slate-50">
                {{ shift.start_time }} – {{ shift.end_time }}
              </div>
              <span v-if="shift.id < 0" class="text-[8px] font-black bg-blue-500 text-white px-1 rounded uppercase">New</span>
            </div>

            <div class="flex items-center gap-3 z-10">
              
              <div v-if="shift.employee_name" class="flex items-center gap-2 bg-blue-50 pl-2 pr-1 py-1 rounded-lg border border-blue-100">
                <span class="text-[11px] font-bold text-blue-700 truncate max-w-[80px]">
                  {{ shift.employee_name }}
                </span>
              ...
              <button 
                ...
                :disabled="userRole === 'admin' || isPastDate(shift.date)"
                :class="[
              ...
              <button v-if="userRole === 'admin'" @click="markForDeletion(shift)" class="ml-2 text-slate-300 active:text-red-500 p-1">
      ...
      <div v-if="Object.keys(groupedShifts).length === 0" class="text-center py-10 opacity-40">
        <p class="text-xs font-bold uppercase">Расписание пусто</p>
      </div>
    </div>

    <div v-if="hasUnsavedChanges && userRole === 'admin'" 
         class="fixed bottom-20 left-4 right-4 z-[100] animate-in slide-in-from-bottom-5 fade-in">
      <button 
        ...
        <span v-if="isSaving" class="w-4 h-4 border-2 border-white/50 border-t-white rounded-full animate-spin"></span>
        <Save v-else class="w-4 h-4" />
        Сохранить изменения ({{ unsavedNewShifts.length + pendingDeletes.size }})
      </button>
    </div>

    <div v-if="isModalOpen" class="fixed inset-0 z-[200] flex items-end justify-center sm:items-center bg-black/40 backdrop-blur-sm p-4">
      <div class="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-5 animate-in slide-in-from-bottom-10 fade-in duration-200">
        <h3 class="text-lg font-black italic text-slate-800 mb-4 uppercase">Новая смена</h3>
        
        <div class="space-y-4">
          <div>
            <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 block mb-1">Дата</label>
            <input type="date" v-model="form.date" class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-blue-500 transition-colors" />
          </div>
          
          <div class="flex gap-3">
            <div class="flex-1">
              <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 block mb-1">Начало</label>
              <input type="time" v-model="form.start_time" class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold text-center outline-none focus:border-blue-500" />
            </div>
            <div class="flex-1">
              <label class="text-[10px] font-bold text-slate-400 uppercase ml-1 block mb-1">Конец</label>
              <input type="time" v-model="form.end_time" class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold text-center outline-none focus:border-blue-500" />
            </div>
          </div>
        </div>

        <div class="flex gap-2 mt-6">
          <button @click="isModalOpen = false" class="flex-1 py-3 text-xs font-bold uppercase text-slate-500 bg-slate-100 rounded-xl">Отмена</button>
          <button @click="addToDraft" class="flex-1 py-3 text-xs font-bold uppercase text-white bg-blue-600 rounded-xl shadow-lg shadow-blue-200">Добавить</button>
        </div>
      </div>
    </div>
